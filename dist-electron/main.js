...
var J = (e, a, i) => xn(e, typeof a != "symbol" ? a + "" : a, i);
import { dialog as ya, app as Fe, BrowserWindow as gi, ipcMain as Pe } from "electron";
import { fileURLToPath as hn } from "node:url";
import Z from "node:path";
import vn, { createWriteStream as bn, rename as gn } from "fs";
import $, { Transform as yn, Readable as wn } from "stream";
import ye, { TextEncoder as _n } from "util";
import yi from "path";
import wa from "http";
import _a from "https";
import Be from "url";
import kn from "assert";
import En from "tty";
import Rn from "os";
import Q from "zlib";
...
class jn {
  constructor(a, i, n) {
    J(this, "writeStream");
    J(this, "headers", []);
    J(this, "batchSize", 5e3);
    J(this, "logger");
    J(this, "tempFilePath");
    J(this, "finalFilePath");
    J(this, "bufferSize", 16384);
    // 16KB buffer for better performance
    J(this, "headersWritten", !1);
    var s;
    this.finalFilePath = `${a}/${i}.csv`, this.tempFilePath = `${a}/.${i}.csv`, this.writeStream = bn(this.tempFilePath, {
      flags: "a",
      highWaterMark: this.bufferSize
    }), this.logger = n, (s = this.logger) == null || s.log(
  ...
  escapeCSVValue(a) {
    if (a == null)
      return '""';
    const i = typeof a == "object" ? JSON.stringify(a) : String(a);
...
`) ? `"${i.replace(/"/g, '""')}"` : i;
  ...
  extractHeaders(a) {
    ...
    return a.forEach((n) => {
      Object.keys(n).forEach((s) => i.add(s));
    }), Array.from(i);
  }
  /**
  ...
  updateHeaders(a) {
    let i = !1;
    return a.forEach((n) => {
      this.headers.includes(n) || (this.headers.push(n), i = !0);
    }), i;
  }
  /**
   ...
   */
  objectToCSVLine(a) {
    return this.headers.map((i) => this.escapeCSVValue(a[i] ?? "")).join(",") + `
`;
  }
  /**
  ...
  writeHeaders() {
    var i;
    const a = this.headers.map((n) => this.escapeCSVValue(n)).join(",") + `
`;
    this.writeStream.write(a), this.headersWritten = !0, (i = this.logger) == null || i.log(`Headers written to CSV file: ${this.headers.join(", ")}`);
  }
  /**
  ...
  async writeData(a) {
    ...
    if (!(!Array.isArray(a) || a.length === 0))
      return (i = this.logger) == null || i.log(`Processing ${a.length} records for CSV`), new Promise((n, s) => {
        var o;
        try {
          const t = this.extractHeaders(a), c = this.updateHeaders(t);
          (!this.headersWritten || c) && this.writeHeaders();
          const d = new yn({
            objectMode: !0,
            transform: (p, m, x) => {
              try {
                const f = this.objectToCSVLine(p);
                x(null, f);
              } catch (f) {
                x(f);
              }
          ...
          const r = () => {
            ...
            if (p.length === 0) {
              d.end(), n(), (x = this.logger) == null || x.log("All data written to CSV");
              return;
            }
            const m = p.every(
              (f) => d.write(f)
            );
            u += p.length, m ? setImmediate(r) : d.once("drain", r);
          };
          d.on("error", (p) => {
            var m;
            (m = this.logger) == null || m.error(`Transform stream error: ${p}`), s(p);
          }), r();
        } catch (t) {
          (o = this.logger) == null || o.error(`Error writing data: ${t}`), s(t);
        }
      });
  }
  /**
  ...
  async writeArrays(a) {
    ...
    try {
      ...
      a.forEach((s) => {
        s.forEach((o) => {
          Object.keys(o).forEach((t) => n.add(t));
        });
      }), this.headers = Array.from(n);
      for (const s of a)
        await this.writeData(s);
    } catch (n) {
      throw (i = this.logger) == null || i.error(`Error writing arrays: ${n}`), n;
    }
  }
  setBatchSize(a) {
    var i;
    ...
    this.batchSize = a, (i = this.logger) == null || i.log(`Batch size set to ${a}`);
  }
  async close() {
    var a;
    return (a = this.logger) == null || a.log("Closing write stream"), new Promise((i, n) => {
      const s = (o) => {
        var t;
        this.headers = [], this.headersWritten = !1, o ? ((t = this.logger) == null || t.error(`Error during close: ${o}`), n(o)) : i();
      };
      this.writeStream.end(() => {
        gn(this.tempFilePath, this.finalFilePath, (o) => {
          var t;
          o ? s(o) : ((t = this.logger) == null || t.log(`File renamed to ${this.finalFilePath}`), s());
        });
      }), this.writeStream.on("error", s);
    });
  }
}
const Cn = [
...
const { toString: On } = Object.prototype, { getPrototypeOf: ka } = Object, Ue = /* @__PURE__ */ ((e) => (a) => {
  const i = On.call(a);
  return e[i] || (e[i] = i.slice(8, -1).toLowerCase());
})(/* @__PURE__ */ Object.create(null)), H = (e) => (e = e.toLowerCase(), (a) => Ue(a) === e), Ne = (e) => (a) => typeof a === e, { isArray: ue } = Array, he = Ne("undefined");
function Fn(e) {
  return e !== null && !he(e) && e.constructor !== null && !he(e.constructor) && N(e.constructor.isBuffer) && e.constructor.isBuffer(e);
}
const _i = H("ArrayBuffer");
function Ln(e) {
  let a;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? a = ArrayBuffer.isView(e) : a = e && e.buffer && _i(e.buffer), a;
}
const An = Ne("string"), N = Ne("function"), ki = Ne("number"), De = (e) => e !== null && typeof e == "object", zn = (e) => e === !0 || e === !1, Te = (e) => {
  if (Ue(e) !== "object")
    return !1;
  const a = ka(e);
  return (a === null || a === Object.prototype || Object.getPrototypeOf(a) === null) && !(Symbol.toStringTag in e) && !(Symbol.iterator in e);
}, qn = H("Date"), Pn = H("File"), Bn = H("Blob"), Un = H("FileList"), Nn = (e) => De(e) && N(e.pipe), Dn = (e) => {
  let a;
  return e && (typeof FormData == "function" && e instanceof FormData || N(e.append) && ((a = Ue(e)) === "formdata" || // detect form-data instance
  a === "object" && N(e.toString) && e.toString() === "[object FormData]"));
}, In = H("URLSearchParams"), [$n, Mn, Hn, Vn] = ["ReadableStream", "Request", "Response", "Headers"].map(H), Wn = (e) => e.trim ? e.trim() : e.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, "");
function we(e, a, { allOwnKeys: i = !1 } = {}) {
...
const ie = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : global, Ri = (e) => !he(e) && e !== ie;
function ta() {
  const { caseless: e } = Ri(this) && this || {}, a = {}, i = (n, s) => {
    const o = e && Ei(a, s) || s;
    Te(a[o]) && Te(n) ? a[o] = ta(a[o], n) : Te(n) ? a[o] = ta({}, n) : ue(n) ? a[o] = n.slice() : a[o] = n;
  };
  for (let n = 0, s = arguments.length; n < s; n++)
    arguments[n] && we(arguments[n], i);
  return a;
}
const Kn = (e, a, i, { allOwnKeys: n } = {}) => (we(a, (s, o) => {
  i && N(s) ? e[o] = wi(s, i) : e[o] = s;
}, { allOwnKeys: n }), e), Jn = (e) => (e.charCodeAt(0) === 65279 && (e = e.slice(1)), e), Gn = (e, a, i, n) => {
  ...
  for (; a-- > 0; )
    i[a] = e[a];
  return i;
}, Qn = /* @__PURE__ */ ((e) => (a) => e && a instanceof e)(typeof Uint8Array < "u" && ka(Uint8Array)), es = (e, a) => {
  ...
  for (; (i = e.exec(a)) !== null; )
    n.push(i);
  return n;
}, is = H("HTMLFormElement"), ns = (e) => e.toLowerCase().replace(
  /[-_\s]([a-z\d])(\w*)/g,
  function(i, n, s) {
    return n.toUpperCase() + s;
  }
), Ia = (({ hasOwnProperty: e }) => (a, i) => e.call(a, i))(Object.prototype), ss = H("RegExp"), Si = (e, a) => {
...
}, cs = (e, a) => e != null && Number.isFinite(e = +e) ? e : a, Ke = "abcdefghijklmnopqrstuvwxyz", $a = "0123456789", ji = {
  DIGIT: $a,
  ALPHA: Ke,
  ALPHA_DIGIT: Ke + Ke.toUpperCase() + $a
}, ps = (e = 16, a = ji.ALPHA_DIGIT) => {
  let i = "";
  const { length: n } = a;
  for (; e--; )
    i += a[Math.random() * n | 0];
...
const us = (e) => {
  ...
  return i(e, 0);
}, ds = H("AsyncFunction"), ms = (e) => e && (De(e) || N(e)) && N(e.then) && N(e.catch), Ci = ((e, a) => e ? setImmediate : a ? ((i, n) => (ie.addEventListener("message", ({ source: s, data: o }) => {
  ...
  N(ie.postMessage)
), fs = typeof queueMicrotask < "u" ? queueMicrotask.bind(ie) : typeof process < "u" && process.nextTick || Ci, l = {
  isArray: ue,
  isArrayBuffer: _i,
  isBuffer: Fn,
  isFormData: Dn,
...
b.from = (e, a, i, n, s, o) => {
  ...
  return l.toFlatObject(e, t, function(d) {
...
var vs = ye, Ai = $.Stream, Ma = hs, bs = j;
function j() {
...
j.prototype.pipe = function(e, a) {
  return Ai.prototype.pipe.call(this, e, a), this.resume(), e;
};
j.prototype._getNext = function() {
...
j.prototype._realGetNext = function() {
...
j.prototype._handleErrors = function(e) {
  var a = this;
...
j.prototype._reset = function() {
  this.writable = !1, this._streams = [], this._currentStream = null;
};
j.prototype._checkDataSize = function() {
  if (this._updateDataSize(), !(this.dataSize <= this.maxDataSize)) {
    var e = "DelayedStream#maxDataSize of " + this.maxDataSize + " bytes exceeded.";
    this._emitError(new Error(e));
  }
};
j.prototype._updateDataSize = function() {
  this.dataSize = 0;
  var e = this;
...
j.prototype._emitError = function(e) {
 ...
 */
(function(e) {
  var a = ys, i = yi.extname, n = /^\s*([^;\s]*)(?:;|\s|$)/, s = /^text\//i;
  ...
  function o(r) {
    if (!r || typeof r != "string")
      return !1;
    var p = n.exec(r), m = p && a[p[1].toLowerCase()];
    return m && m.charset ? m.charset : p && s.test(p[1]) ? "UTF-8" : !1;
  }
  function t(r) {
    if (!r || typeof r != "string")
      return !1;
  ...
  function c(r) {
    if (!r || typeof r != "string")
      return !1;
    var p = n.exec(r), m = p && e.extensions[p[1].toLowerCase()];
    return !m || !m.length ? !1 : m[0];
  }
  function d(r) {
    if (!r || typeof r != "string")
      return !1;
    var p = i("x." + r).toLowerCase().substr(1);
    ...
    Object.keys(a).forEach(function(f) {
      ...
      if (!(!h || !h.length)) {
        ...
        for (var g = 0; g < h.length; g++) {
          var w = h[g];
          if (p[w]) {
            var y = m.indexOf(a[p[w]].source), S = m.indexOf(v.source);
            if (p[w] !== "application/octet-stream" && (y > S || y === S && p[w].substr(0, 12) === "application/"))
              continue;
          }
          p[w] = f;
        }
...
var Va = qi, Ss = Pi, Bi = js;
function js(e, a, i, n) {
  var s = i.keyedList ? i.keyedList[i.index] : i.index;
  i.jobs[s] = Cs(a, s, e[s], function(o, t) {
    s in i.jobs && (delete i.jobs[s], o ? Ss(i) : i.results[s] = t, n(o, i.results));
  });
}
function Cs(e, a, i, n) {
  var s;
  return e.length == 2 ? s = e(i, Va(n)) : s = e(i, a, Va(n)), s;
}
var Ui = Ts;
function Ts(e, a) {
...
var Os = Pi, Fs = qi, Ni = Ls;
function Ls(e) {
  Object.keys(this.jobs).length && (this.index = this.size, Os(this), Fs(e)(null, this.results));
}
var As = Bi, zs = Ui, qs = Ni, Ps = Bs;
function Bs(e, a, i) {
  for (var n = zs(e); n.index < (n.keyedList || e).length; )
    As(e, a, n, function(s, o) {
      if (s) {
        i(s, o);
        return;
      }
...
var Ie = { exports: {} }, Wa = Bi, Us = Ui, Ns = Ni;
Ie.exports = Ds;
Ie.exports.ascending = Di;
Ie.exports.descending = Is;
function Ds(e, a, i, n) {
  var s = Us(e, i);
  return Wa(e, a, s, function o(t, c) {
    if (t) {
      n(t, c);
      return;
    }
...
}, Ea = bs, $i = ye, Je = yi, Ks = wa, Js = _a, Gs = Be.parse, Xs = vn, Ys = $.Stream, Ge = zi, Zs = Vs, ra = Ws, Qs = R;
$i.inherits(R, Ea);
function R(e) {
...
R.prototype._trackLength = function(e, a, i) {
  var n = 0;
  i.knownLength != null ? n += +i.knownLength : Buffer.isBuffer(a) ? n = a.length : typeof a == "string" && (n = Buffer.byteLength(a)), this._valueLength += n, this._overheadLength += Buffer.byteLength(e) + R.LINE_BREAK.length, !(!a || !a.path && !(a.readable && a.hasOwnProperty("httpVersion")) && !(a instanceof Ys)) && (i.knownLength || this._valuesToMeasure.push(a));
};
R.prototype._lengthRetriever = function(e, a) {
  e.hasOwnProperty("fd") ? e.end != null && e.end != 1 / 0 && e.start != null ? a(null, e.end + 1 - (e.start ? e.start : 0)) : Xs.stat(e.path, function(i, n) {
    var s;
    if (i) {
      a(i);
      return;
...
R.prototype._getContentType = function(e, a) {
  var i = a.contentType;
  return !i && e.name && (i = Ge.lookup(e.name)), !i && e.path && (i = Ge.lookup(e.path)), !i && e.readable && e.hasOwnProperty("httpVersion") && (i = e.headers["content-type"]), !i && (a.filepath || a.filename) && (i = Ge.lookup(a.filepath || a.filename)), !i && typeof e == "object" && (i = R.DEFAULT_CONTENT_TYPE), i;
};
...
R.prototype.getLength = function(e) {
  ...
  Zs.parallel(this._valuesToMeasure, this._lengthRetriever, function(i, n) {
    if (i) {
      e(i);
      return;
    }
...
R.prototype.submit = function(e, a) {
  ...
  return typeof e == "string" ? (e = Gs(e), n = ra({
    port: e.port,
    path: e.pathname,
    host: e.hostname,
    protocol: e.protocol
  }, s)) : (n = ra(e, s), n.port || (n.port = n.protocol == "https:" ? 443 : 80)), n.headers = this.getHeaders(e.headers), n.protocol == "https:" ? i = Js.request(n) : i = Ks.request(n), this.getLength((function(o, t) {
...
function eo(e) {
  return l.isArray(e) && !e.some(ca);
}
const ao = l.toFlatObject(l, {}, null, function(a) {
  return /^is[A-Z]/.test(a);
});
function $e(e, a, i) {
  if (!l.isObject(e))
    throw new TypeError("target must be an object");
  a = a || new (Mi || FormData)(), i = l.toFlatObject(i, {
    metaTokens: !0,
    dots: !1,
    indexes: !1
  }, !1, function(v, h) {
    return !l.isUndefined(h[v]);
  });
  const n = i.metaTokens, s = i.visitor || r, o = i.dots, t = i.indexes, d = (i.Blob || typeof Blob < "u" && Blob) && l.isSpecCompliantForm(a);
  if (!l.isFunction(s))
  ...
  function r(f, v, h) {
    ...
    if (f && !h && typeof f == "object") {
      ...
      else if (l.isArray(f) && eo(f) || (l.isFileList(f) || l.endsWith(v, "[]")) && (g = l.toArray(f)))
        return v = Hi(v), g.forEach(function(y, S) {
          !(l.isUndefined(y) || y === null) && a.append(
            // eslint-disable-next-line no-nested-ternary
            t === !0 ? Ka([v], S, o) : t === null ? v : v + "[]",
            u(y)
          );
        }), !1;
    }
    return ca(f) ? !0 : (a.append(Ka(h, v, o), u(f)), !1);
  }
  const p = [], m = Object.assign(ao, {
    defaultVisitor: r,
    convertValue: u,
    isVisitable: ca
  });
  function x(f, v) {
    if (!l.isUndefined(f)) {
      ...
      p.push(f), l.forEach(f, function(g, w) {
...
function Ra(e, a, i) {
  if (!a)
    return e;
  const n = i && i.encode || io, s = i && i.serialize;
  let o;
  if (s ? o = s(a, i) : o = l.isURLSearchParams(a) ? a.toString() : new Vi(a, i).toString(n), o) {
    const t = e.indexOf("#");
...
}, ja = typeof window < "u" && typeof document < "u", pa = typeof navigator == "object" && navigator || void 0, oo = ja && (!pa || ["ReactNative", "NativeScript", "NS"].indexOf(pa.product) < 0), to = typeof WorkerGlobalScope < "u" && // eslint-disable-next-line no-undef
self instanceof WorkerGlobalScope && typeof self.importScripts == "function", ro = ja && window.location.href || "http://localhost", co = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  hasBrowserEnv: ja,
...
function po(e, a) {
  return $e(e, new O.classes.URLSearchParams(), Object.assign({
    visitor: function(i, n, s, o) {
      return O.isNode && l.isBuffer(i) ? (this.append(n, i.toString("base64")), !1) : o.defaultVisitor.apply(this, arguments);
    }
  }, a));
}
function lo(e) {
...
function Ki(e) {
  function a(i, n, s, o) {
    ...
    return t = !t && l.isArray(s) ? s.length : t, d ? (l.hasOwnProp(s, t) ? s[t] = [s[t], n] : s[t] = n, !c) : ((!s[t] || !l.isObject(s[t])) && (s[t] = []), a(i, n, s[t], o) && l.isArray(s[t]) && (s[t] = uo(s[t])), !c);
  }
...
const _e = {
  ...
  transformRequest: [function(a, i) {
    const n = i.getContentType() || "", s = n.indexOf("application/json") > -1, o = l.isObject(a);
    if (o && l.isHTMLForm(a) && (a = new FormData(a)), l.isFormData(a))
      return s ? JSON.stringify(Ki(a)) : a;
    ...
    if (o) {
      if (n.indexOf("application/x-www-form-urlencoded") > -1)
        return po(a, this.formSerializer).toString();
      if ((c = l.isFileList(a)) || n.indexOf("multipart/form-data") > -1) {
    ...
    return o || s ? (i.setContentType("application/json", !1), mo(a)) : a;
  }],
  transformResponse: [function(a) {
    ...
    if (l.isResponse(a) || l.isReadableStream(a))
      return a;
    if (a && l.isString(a) && (n && !this.responseType || s)) {
...
const fo = l.toObjectSet([
  ...
  return e && e.split(`
`).forEach(function(t) {
    s = t.indexOf(":"), i = t.substring(0, s).trim().toLowerCase(), n = t.substring(s + 1).trim(), !(!i || a[i] && fo[i]) && (i === "set-cookie" ? a[i] ? a[i].push(n) : a[i] = [n] : a[i] = a[i] ? a[i] + ", " + n : n);
  }), a;
}, Xa = Symbol("internals");
function me(e) {
...
class L {
  ...
  delete(a, i) {
    ...
    function o(t) {
      if (t = me(t), t) {
        const c = l.findKey(n, t);
        c && (!i || Xe(n, n[c], c, i)) && (delete n[c], s = !0);
      }
    }
    return l.isArray(a) ? a.forEach(o) : o(a), s;
  }
  clear(a) {
  ...
  normalize(a) {
    ...
    return l.forEach(this, (s, o) => {
      const t = l.findKey(n, o);
      if (t) {
        i[t] = Oe(s), delete i[o];
        return;
      }
      const c = a ? bo(o) : String(o).trim();
      c !== o && delete i[o], i[c] = Oe(s), n[c] = !0;
    }), this;
  }
  concat(...a) {
  ...
  toJSON(a) {
    ...
    return l.forEach(this, (n, s) => {
      n != null && n !== !1 && (i[s] = a && l.isArray(n) ? n.join(", ") : n);
    }), i;
  }
  [Symbol.iterator]() {
...
`);
  ...
  static accessor(a) {
    ...
    return l.isArray(a) ? a.forEach(o) : o(a), this;
  }
}
...
var _o = Be.parse, ko = {
  ...
  ws: 80,
  wss: 443
}, Eo = String.prototype.endsWith || function(e) {
  return e.length <= this.length && this.indexOf(e, this.length - e.length) !== -1;
};
function Ro(e) {
  var a = typeof e == "string" ? _o(e) : e || {}, i = a.protocol, n = a.host, s = a.port;
  if (typeof n != "string" || !n || typeof i != "string" || (i = i.split(":", 1)[0], n = n.replace(/:\d*$/, ""), s = parseInt(s) || ko[i] || 0, !So(n, s)))
    return "";
...
function So(e, a) {
  ...
  return i ? i === "*" ? !1 : i.split(/[,\s]/).every(function(n) {
    ...
    return t && t !== a ? !0 : /^[.*]/.test(o) ? (o.charAt(0) === "*" && (o = o.slice(1)), !Eo.call(e, o)) : e !== o;
  }) : !0;
}
function pe(e) {
  return process.env[e.toLowerCase()] || process.env[e.toUpperCase()] || "";
}
var jo = Ro, Ta = { exports: {} }, Se = { exports: {} }, je = { exports: {} }, Ze, Ya;
function Co() {
  if (Ya) return Ze;
  Ya = 1;
  ...
  function t(r) {
    if (r = String(r), !(r.length > 100)) {
      ...
      if (p) {
        var m = parseFloat(p[1]), x = (p[2] || "ms").toLowerCase();
        switch (x) {
          case "years":
          case "year":
          case "yrs":
  ...
  function c(r) {
    var p = Math.abs(r);
    return p >= n ? Math.round(r / n) + "d" : p >= i ? Math.round(r / i) + "h" : p >= a ? Math.round(r / a) + "m" : p >= e ? Math.round(r / e) + "s" : r + "ms";
  }
  function d(r) {
    var p = Math.abs(r);
    return p >= n ? u(r, p, n, "day") : p >= i ? u(r, p, i, "hour") : p >= a ? u(r, p, a, "minute") : p >= e ? u(r, p, e, "second") : r + " ms";
  }
  function u(r, p, m, x) {
    var f = p >= m * 1.5;
...
function Gi() {
  ...
  Za = 1;
  function e(a) {
    n.debug = n, n.default = n, n.coerce = u, n.disable = t, n.enable = o, n.enabled = c, n.humanize = Co(), n.destroy = r, Object.keys(a).forEach((p) => {
      n[p] = a[p];
    }), n.names = [], n.skips = [], n.formatters = {};
    function i(p) {
      let m = 0;
      for (let x = 0; x < p.length; x++)
        m = (m << 5) - m + p.charCodeAt(x), m |= 0;
    ...
    function n(p) {
      ...
      function h(...g) {
        if (!h.enabled)
          return;
        const w = h, y = Number(/* @__PURE__ */ new Date()), S = y - (m || y);
      ...
      return h.namespace = p, h.useColors = n.useColors(), h.color = n.selectColor(p), h.extend = s, h.destroy = n.destroy, Object.defineProperty(h, "enabled", {
        enumerable: !0,
        configurable: !1,
        get: () => x !== null ? x : (f !== n.namespaces && (f = n.namespaces, v = n.enabled(p)), v),
        set: (g) => {
          x = g;
        }
    ...
    function s(p, m) {
      const x = n(this.namespace + (typeof m > "u" ? ":" : m) + p);
      return x.log = this.log, x;
    }
    function o(p) {
      n.save(p), n.namespaces = p, n.names = [], n.skips = [];
      let m;
      const x = (typeof p == "string" ? p : "").split(/[\s,]+/), f = x.length;
      for (m = 0; m < f; m++)
        x[m] && (p = x[m].replace(/\*/g, ".*?"), p[0] === "-" ? n.skips.push(new RegExp("^" + p.slice(1) + "$")) : n.names.push(new RegExp("^" + p + "$")));
    }
    function t() {
      const p = [
        ...n.names.map(d),
    ...
    function c(p) {
      if (p[p.length - 1] === "*")
        return !0;
      let m, x;
      for (m = 0, x = n.skips.length; m < x; m++)
        if (n.skips[m].test(p))
          return !1;
      for (m = 0, x = n.names.length; m < x; m++)
        if (n.names[m].test(p))
          return !0;
      return !1;
    }
...
var Ce = { exports: {} }, ea, ei;
function Oo() {
  return ei || (ei = 1, ea = (e, a) => {
    a = a || process.argv;
...
var ni;
function Ao() {
  return ni || (ni = 1, typeof process > "u" || process.type === "renderer" || process.browser === !0 || process.__nwjs ? Se.exports = To() : Se.exports = Lo()), Se.exports;
}
var fe, zo = function() {
  if (!fe) {
    try {
      fe = Ao()("follow-redirects");
    } catch {
    }
...
}, ke = Be, ve = ke.URL, qo = wa, Po = _a, Oa = $.Writable, Fa = kn, Xi = zo;
(function() {
...
var Bo = [
  ...
  "search",
  "hash"
], Aa = ["abort", "aborted", "connect", "error", "socket", "timeout"], za = /* @__PURE__ */ Object.create(null);
...
var la = Ee(
  ...
  "write after end"
), Io = Oa.prototype.destroy || Zi;
function B(e, a) {
  Oa.call(this), this._sanitizeOptions(e), this._options = e, this._ended = !1, this._ending = !1, this._redirectCount = 0, this._redirects = [], this._requestBodyLength = 0, this._requestBodyBuffers = [], a && this.on("response", a);
  var i = this;
...
B.prototype = Object.create(Oa.prototype);
B.prototype.abort = function() {
  Pa(this._currentRequest), this._currentRequest.abort(), this.emit("abort");
};
B.prototype.destroy = function(e) {
  return Pa(this._currentRequest, e), Io.call(this, e), this;
};
B.prototype.write = function(e, a, i) {
  ...
  this._requestBodyLength + e.length <= this._options.maxBodyLength ? (this._requestBodyLength += e.length, this._requestBodyBuffers.push({ data: e, encoding: a }), this._currentRequest.write(e, a, i)) : (this.emit("error", new No()), this.abort());
};
...
B.prototype._performRequest = function() {
  ...
  for (var s of Aa)
    n.on(s, za[s]);
  if (this._currentUrl = /^\//.test(this._options.path) ? ke.format(this._options) : (
...
B.prototype._processResponse = function(e) {
  ...
  if (Pa(this._currentRequest), e.destroy(), ++this._redirectCount > this._options.maxRedirects)
    throw new Uo();
  ...
  var t = ia(/^host$/i, this._options.headers), c = qa(this._currentUrl), d = t || c.host, u = /^\w+:/.test(i) ? this._currentUrl : ke.format(Object.assign(c, { host: d })), r = $o(i, u);
  if (Xi("redirecting to", r.href), this._isRedirect = !0, da(r, this._options), (r.protocol !== c.protocol && r.protocol !== "https:" || r.host !== d && !Mo(r.host, d)) && ia(/^(?:(?:proxy-)?authorization|cookie)$/i, this._options.headers), oe(s)) {
    var p = {
      headers: e.headers,
      statusCode: a
    }, m = {
    ...
    s(this._options, p, m), this._sanitizeOptions(this._options);
  }
  this._performRequest();
};
function Yi(e) {
  ...
  return Object.keys(e).forEach(function(n) {
    ...
    function c(u, r, p) {
      return Vo(u) ? u = da(u) : ne(u) ? u = da(qa(u)) : (p = r, r = Qi(u), u = { protocol: s }), oe(r) && (p = r, r = null), r = Object.assign({
        maxRedirects: a.maxRedirects,
        maxBodyLength: a.maxBodyLength
      }, u, r), r.nativeProtocols = i, !ne(r.host) && !ne(r.hostname) && (r.hostname = "::1"), Fa.equal(r.protocol, s, "protocol mismatch"), Xi("options", r), new B(r, p);
    }
    function d(u, r, p) {
      var m = t.request(u, r, p);
      return m.end(), m;
    }
...
function qa(e) {
  ...
  if (La)
    a = new ve(e);
  else if (a = Qi(ke.parse(e)), !ne(a.protocol))
    throw new la({ input: e });
  return a;
}
function $o(e, a) {
  return La ? new ve(e, a) : qa(ke.resolve(a, e));
}
function Qi(e) {
...
function Vo(e) {
  return ve && e instanceof ve;
}
Ta.exports = Yi({ http: qo, https: Po });
Ta.exports.wrap = Yi;
var Wo = Ta.exports;
const Ko = /* @__PURE__ */ Fi(Wo), Le = "1.7.7";
function en(e) {
...
class si extends $.Transform {
  constructor(a) {
    a = l.toFlatObject(a, {
      ...
      ticksRate: 2,
      samplesCount: 15
    }, null, (n, s) => !l.isUndefined(s[n])), super({
  ...
  _transform(a, i, n) {
    const s = this[na], o = s.maxRate, t = this.readableHighWaterMark, c = s.timeWindow, d = 1e3 / c, u = o / d, r = s.minChunkSize !== !1 ? Math.max(s.minChunkSize, u * 0.01) : 0, p = (x, f) => {
      const v = Buffer.byteLength(x);
      s.bytesSeen += v, s.bytes += v, s.isCaptured && this.emit("progress", s.bytesSeen), this.push(x) ? process.nextTick(f) : s.onReadCallback = () => {
        s.onReadCallback = null, process.nextTick(f);
      };
    }, m = (x, f) => {
      const v = Buffer.byteLength(x);
      let h = null, g = t, w, y = 0;
      if (o) {
        const S = Date.now();
      ...
      g && v > g && v - g > r && (h = x.subarray(g), x = x.subarray(0, g)), p(x, h ? () => {
...
const { asyncIterator: oi } = Symbol, an = async function* (e) {
  e.stream ? yield* e.stream() : e.arrayBuffer ? yield await e.arrayBuffer() : e[oi] ? yield* e[oi]() : yield e;
}, Xo = l.ALPHABET.ALPHA_DIGIT + "-_", be = new _n(), Y = `\r
`, Yo = be.encode(Y), Zo = 2;
class Qo {
  constructor(a, i) {
    const { escapeName: n } = this.constructor, s = l.isString(i);
    let o = `Content-Disposition: form-data; name="${n(a)}"${!s && i.name ? `; filename="${n(i.name)}"` : ""}${Y}`;
    s ? i = be.encode(String(i).replace(/\r?\n|\r\n?/g, Y)) : o += `Content-Type: ${i.type || "application/octet-stream"}${Y}`, this.headers = be.encode(o + Y), this.contentLength = s ? i.byteLength : i.size, this.size = this.headers.byteLength + this.contentLength + Zo, this.name = a, this.value = i;
  }
  async *encode() {
    yield this.headers;
    const { value: a } = this;
    l.isTypedArray(a) ? yield a : yield* an(a), yield Yo;
  }
  static escapeName(a) {
...
const et = (e, a, i) => {
  const {
    ...
    size: s = 25,
    boundary: o = n + "-" + l.generateString(s, Xo)
  } = i || {};
  if (!l.isFormData(e))
    throw TypeError("FormData instance required");
  if (o.length < 1 || o.length > 70)
    throw Error("boundary must be 10-70 characters long");
  const t = be.encode("--" + o + Y), c = be.encode("--" + o + "--" + Y + Y);
  let d = c.byteLength;
  const u = Array.from(e.entries()).map(([p, m]) => {
    const x = new Qo(p, m);
    return d += x.size, x;
  });
  d += t.byteLength * u.length, d = l.toFiniteNumber(d);
  const r = {
    "Content-Type": `multipart/form-data; boundary=${o}`
  };
  return Number.isFinite(d) && (r["Content-Length"] = d), a && a(r), wn.from(async function* () {
    for (const p of u)
      yield t, yield* p.encode();
    yield c;
  }());
};
class at extends $.Transform {
  __transform(a, i, n) {
    this.push(a), n();
  }
  _transform(a, i, n) {
...
const it = (e, a) => l.isAsyncFn(e) ? function(...i) {
  const n = i.pop();
...
function st(e, a) {
  ...
  return [(...u) => {
    const r = Date.now(), p = r - i;
    p >= n ? t(u, r) : (s = u, o || (o = setTimeout(() => {
      o = null, t(s);
    }, n - p)));
  }, () => s && t(s)];
}
...
}, ri = l.isFunction(Q.createBrotliDecompress), { http: tt, https: rt } = Ko, ct = /https:?/, ci = O.protocols.map((e) => e + ":"), pi = (e, [a, i]) => (e.on("end", i).on("error", i), a);
function pt(e, a) {
...
const lt = typeof process < "u" && l.kindOf(process) === "process", ut = (e) => new Promise((a, i) => {
  let n, s;
  const o = (d, u) => {
    s || (s = !0, n && n(d, u));
  }, t = (d) => {
...
}, li = (e, a) => dt(l.isObject(e) ? e : { address: e, family: a }), mt = lt && function(a) {
  return ut(async function(n, s, o) {
    let { data: t, lookup: c, family: d } = a;
    const { responseType: u, responseEncoding: r } = a, p = a.method.toUpperCase();
    let m, x = !1, f;
    if (c) {
      const k = it(c, (_) => l.isArray(_) ? _ : [_]);
      c = (_, A, ae) => {
        k(_, A, (F, G, Ve) => {
          if (F)
            return ae(F);
          const K = l.isArray(G) ? G.map((D) => li(D)) : [li(G, Ve)];
          A.all ? ae(F, K) : ae(F, K[0].address, K[0].family);
        });
      };
    }
    const v = new Sn(), h = () => {
    ...
    function g(k) {
      v.emit("abort", !k || k.type ? new ee(null, a, f) : k);
    }
    ...
    const w = Ca(a.baseURL, a.url), y = new URL(w, O.hasBrowserEnv ? O.origin : void 0), S = y.protocol || ci[0];
    if (S === "data:") {
      let k;
      if (p !== "GET")
        return ce(n, s, {
      ...
      return u === "text" ? (k = k.toString(r), (!r || r === "utf8") && (k = l.stripBOM(k))) : u === "stream" && (k = $.Readable.from(k)), ce(n, s, {
        data: k,
        status: 200,
        statusText: "OK",
        headers: new L(),
    ...
    } else if (l.isFormData(t) && l.isFunction(t.getHeaders)) {
      if (E.set(t.getHeaders()), !E.hasContentLength())
        try {
          const k = await ye.promisify(t.getLength).call(t);
          Number.isFinite(k) && k >= 0 && E.setContentLength(k);
        } catch {
        }
    } else if (l.isBlob(t))
      t.size && E.setContentType(t.type || "application/octet-stream"), E.setContentLength(t.size || 0), t = $.Readable.from(an(t));
    else if (t && !l.isStream(t)) {
      if (!Buffer.isBuffer(t)) if (l.isArrayBuffer(t))
    ...
    const mn = l.toFiniteNumber(E.getContentLength());
    l.isArray(M) ? (W = M[0], re = M[1]) : W = re = M, t && (U || W) && (l.isStream(t) || (t = $.Readable.from(t, { objectMode: !1 })), t = $.pipeline([t, new si({
      maxRate: l.toFiniteNumber(W)
    })], l.noop), U && t.on("progress", pi(
      t,
      Ae(
        mn,
        le(ze(U), !1, 3)
    ...
    !l.isUndefined(c) && (P.lookup = c), a.socketPath ? P.socketPath = a.socketPath : (P.hostname = y.hostname.startsWith("[") ? y.hostname.slice(1, -1) : y.hostname, P.port = y.port, nn(P, a.proxy, S + "//" + y.hostname + (y.port ? ":" + y.port : "") + P.path));
    let Re;
    const He = ct.test(P.protocol);
    if (P.agent = He ? a.httpsAgent : a.httpAgent, a.transport ? Re = a.transport : a.maxRedirects === 0 ? Re = He ? _a : wa : (a.maxRedirects && (P.maxRedirects = a.maxRedirects), a.beforeRedirect && (P.beforeRedirects.config = a.beforeRedirect), Re = He ? rt : tt), a.maxBodyLength > -1 ? P.maxBodyLength = a.maxBodyLength : P.maxBodyLength = 1 / 0, a.insecureHTTPParser && (P.insecureHTTPParser = a.insecureHTTPParser), f = Re.request(P, function(_) {
      if (f.destroyed) return;
      const A = [_], ae = +_.headers["content-length"];
      if (z || re) {
        const D = new si({
          maxRate: l.toFiniteNumber(re)
        });
        z && D.on("progress", pi(
          D,
          Ae(
      ...
      if (a.decompress !== !1 && _.headers["content-encoding"])
        switch ((p === "HEAD" || _.statusCode === 204) && delete _.headers["content-encoding"], (_.headers["content-encoding"] || "").toLowerCase()) {
          ...
          case "compress":
          case "x-compress":
            A.push(Q.createUnzip(ti)), delete _.headers["content-encoding"];
            break;
          case "deflate":
            A.push(new at()), A.push(Q.createUnzip(ti)), delete _.headers["content-encoding"];
            break;
          case "br":
            ri && (A.push(Q.createBrotliDecompress(ot)), delete _.headers["content-encoding"]);
        }
      F = A.length > 1 ? $.pipeline(A, l.noop) : A[0];
      const Ve = $.finished(F, () => {
        Ve(), h();
      }), K = {
        status: _.statusCode,
        statusText: _.statusMessage,
        headers: new L(_.headers),
        config: a,
        request: G
      };
      if (u === "stream")
        ...
        F.on("data", function(q) {
          D.push(q), Da += q.length, a.maxContentLength > -1 && Da > a.maxContentLength && (x = !0, F.destroy(), s(new b(
          ...
          f.destroyed || s(b.from(q, null, a, G));
        }), F.on("end", function() {
          try {
            let q = D.length === 1 ? D[0] : Buffer.concat(D);
            u !== "arraybuffer" && (q = q.toString(r), (!r || r === "utf8") && (q = l.stripBOM(q))), K.data = q;
          } catch (q) {
            return s(b.from(q, null, a, K.request, K));
          }
          ce(n, s, K);
        });
      }
    ...
    if (l.isStream(t)) {
      ...
      t.on("end", () => {
        ...
        !k && !_ && g(new ee("Request stream has been aborted", a, f));
      }), t.pipe(f);
    } else
      f.end(t);
  });
}, ft = O.hasStandardBrowserEnv ? (
...
), xt = O.hasStandardBrowserEnv ? (
  // Standard browser envs support document.cookie
  {
    write(e, a, i, n, s, o) {
      const t = [e + "=" + encodeURIComponent(a)];
      l.isNumber(i) && t.push("expires=" + new Date(i).toGMTString()), l.isString(n) && t.push("path=" + n), l.isString(s) && t.push("domain=" + s), o === !0 && t.push("secure"), document.cookie = t.join("; ");
    },
    read(e) {
...
), ui = (e) => e instanceof L ? { ...e } : e;
function te(e, a) {
  a = a || {};
  const i = {};
  function n(u, r, p) {
    return l.isPlainObject(u) && l.isPlainObject(r) ? l.merge.call({ caseless: p }, u, r) : l.isPlainObject(r) ? l.merge({}, r) : l.isArray(r) ? r.slice() : r;
  }
  function s(u, r, p) {
    if (l.isUndefined(r)) {
      if (!l.isUndefined(u))
  ...
  const d = {
    ...
    responseEncoding: t,
    validateStatus: c,
    headers: (u, r) => s(ui(u), ui(r), !0)
  };
  return l.forEach(Object.keys(Object.assign({}, e, a)), function(r) {
    const p = d[r] || s, m = p(e[r], a[r], r);
    l.isUndefined(m) && p !== c || (i[r] = m);
  }), i;
}
const sn = (e) => {
  const a = te({}, e);
  let { data: i, withXSRFToken: n, xsrfHeaderName: s, xsrfCookieName: o, headers: t, auth: c } = a;
  a.headers = t = L.from(t), a.url = Ra(Ca(a.baseURL, a.url), e.params, e.paramsSerializer), c && t.set(
    "Authorization",
  ...
  if (l.isFormData(i)) {
    if (O.hasStandardBrowserEnv || O.hasStandardBrowserWebWorkerEnv)
      ...
      const [u, ...r] = d ? d.split(";").map((p) => p.trim()).filter(Boolean) : [];
      t.setContentType([u || "multipart/form-data", ...r].join("; "));
    }
  }
  if (O.hasStandardBrowserEnv && (n && l.isFunction(n) && (n = n(a)), n || n !== !1 && ft(a.url))) {
    const u = s && o && xt.read(o);
    u && t.set(s, u);
  }
  return a;
}, ht = typeof XMLHttpRequest < "u", vt = ht && function(e) {
  return new Promise(function(i, n) {
    const s = sn(e);
    let o = s.data;
    const t = L.from(s.headers).normalize();
    let { responseType: c, onUploadProgress: d, onDownloadProgress: u } = s, r, p, m, x, f;
    function v() {
      x && x(), f && f(), s.cancelToken && s.cancelToken.unsubscribe(r), s.signal && s.signal.removeEventListener("abort", r);
    }
    ...
    "onloadend" in h ? h.onloadend = g : h.onreadystatechange = function() {
      ...
      n(new b("Network Error", b.ERR_NETWORK, e, h)), h = null;
    }, h.ontimeout = function() {
      let S = s.timeout ? "timeout of " + s.timeout + "ms exceeded" : "timeout exceeded";
      const E = s.transitional || Sa;
    ...
    }, o === void 0 && t.setContentType(null), "setRequestHeader" in h && l.forEach(t.toJSON(), function(S, E) {
      h.setRequestHeader(E, S);
    }), l.isUndefined(s.withCredentials) || (h.withCredentials = !!s.withCredentials), c && c !== "json" && (h.responseType = s.responseType), u && ([m, f] = le(u, !0), h.addEventListener("progress", m)), d && h.upload && ([p, x] = le(d), h.upload.addEventListener("progress", p), h.upload.addEventListener("loadend", x)), (s.cancelToken || s.signal) && (r = (y) => {
      h && (n(!y || y.type ? new ee(null, e, h) : y), h.abort(), h = null);
  ...
  if (a || i) {
    ...
    const o = function(u) {
      if (!s) {
        ...
        n.abort(r instanceof b ? r : new ee(r instanceof Error ? r.message : r));
      }
    };
    let t = a && setTimeout(() => {
...
}, yt = async function* (e, a) {
  for await (const i of wt(e))
    yield* gt(i, a);
}, wt = async function* (e) {
...
}, Me = typeof fetch == "function" && typeof Request == "function" && typeof Response == "function", on = Me && typeof ReadableStream == "function", _t = Me && (typeof TextEncoder == "function" ? /* @__PURE__ */ ((e) => (a) => e.encode(a))(new TextEncoder()) : async (e) => new Uint8Array(await new Response(e).arrayBuffer())), tn = (e, ...a) => {
  try {
    return !!e(...a);
  } catch {
    return !1;
  ...
  return e && !a;
}), mi = 64 * 1024, ma = on && tn(() => l.isReadableStream(new Response("").body)), qe = {
  stream: ma && ((e) => e.body)
};
Me && ((e) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((a) => {
    !qe[a] && (qe[a] = l.isFunction(e[a]) ? (i) => i[a]() : (i, n) => {
...
const Et = async (e) => {
  ...
  const i = l.toFiniteNumber(e.getContentLength());
  return i ?? Et(a);
}, St = Me && (async (e) => {
  let {
    url: a,
  ...
  let x = bt([s, o && o.toAbortSignal()], t), f;
  const v = x && x.unsubscribe && (() => {
    x.unsubscribe();
  });
  let h;
  try {
    if (d && kt && i !== "get" && i !== "head" && (h = await Rt(r, n)) !== 0) {
      let E = new Request(a, {
        method: "POST",
        body: n,
        duplex: "half"
      }), U;
      if (l.isFormData(n) && (U = E.headers.get("content-type")) && r.setContentType(U), E.body) {
        const [z, M] = Ae(
          h,
          le(ze(d))
        );
    ...
    if (ma && (c || y && v)) {
      ...
      const U = l.toFiniteNumber(w.headers.get("content-length")), [z, M] = c && Ae(
        U,
        le(ze(c), !0)
      ) || [];
      w = new Response(
    ...
    u = u || "text";
    let S = await qe[l.findKey(qe, u) || "text"](w, e);
...
const fi = (e) => `- ${e}`, jt = (e) => l.isFunction(e) || e === null || e === !1, rn = {
  getAdapter: (e) => {
    e = l.isArray(e) ? e : [e];
    const { length: a } = e;
    let i, n;
    ...
    for (let o = 0; o < a; o++) {
      i = e[o];
      let t;
      if (n = i, !jt(i) && (n = fa[(t = String(i)).toLowerCase()], n === void 0))
...
function xi(e) {
  return sa(e), e.headers = L.from(e.headers), e.data = Ye.call(
    e,
    e.transformRequest
    ...
    return Ji(n) || (sa(e), n && n.response && (n.response.data = Ye.call(
      e,
      e.transformResponse,
      n.response
...
const hi = {};
Ba.transitional = function(a, i, n) {
  function s(o, t) {
  ...
  return (o, t, c) => {
    ...
    return i && !hi[t] && (hi[t] = !0, console.warn(
      s(
        t,
...
` + o) : n.stack = o;
  ...
  _request(a, i) {
    ...
    }, !1), s != null && (l.isFunction(s) ? i.paramsSerializer = {
      serialize: s
    } : xa.assertOptions(s, {
      encode: X.function,
    ...
    if (!d) {
      ...
      for (f.unshift.apply(f, c), f.push.apply(f, u), m = f.length, r = Promise.resolve(i); p < m; )
        r = r.then(f[p++], f[p++]);
      return r;
    }
    m = c.length;
  ...
  getUri(a) {
    ...
    const i = Ca(a.baseURL, a.url);
    return Ra(i, a.params, a.paramsSerializer);
  }
}
...
class Ua {
  constructor(a) {
    ...
    this.promise.then((s) => {
      ...
      n.reason || (n.reason = new ee(o, t, c), i(n.reason));
    });
  }
  /**
...
function cn(e) {
  ...
  return l.extend(i, se.prototype, a, { allOwnKeys: !0 }), l.extend(i, a, null, { allOwnKeys: !0 }), i.create = function(s) {
    return cn(te(e, s));
  }, i;
}
const T = cn(_e);
...
T.mergeConfig = te;
T.AxiosHeaders = L;
T.formToJSON = (e) => Ki(l.isHTMLForm(e) ? new FormData(e) : e);
...
class At {
  ...
  log(a) {
    this.Callback("details", a);
  }
  warn(a) {
    this.Callback("warn", a);
  }
}
let C = null, va = null;
function zt(e) {
  return va = e, C = new At(e), C;
}
function vi(e, a) {
  va && va(e, a);
}
async function pn(e, a) {
  C == null || C.log("searching _promoscore ....."), a || (a = e == "articles" ? Tn : Cn);
  const { data: i } = await Lt(a, e);
  return i;
}
let oa = [];
function qt(e, a) {
  return e.map((i) => {
    i = Bt(i, a);
    const n = { ...i };
    return i.product_price && (n.product_price = (parseFloat(i.product_price) / 100).toFixed(2)), i.product_unit_price && (n.product_unit_price = (parseFloat(i.product_unit_price) / 100).toFixed(2)), n;
  });
}
const Pt = {
...
function Bt(e, a) {
  if (!Array.isArray(e)) {
    const i = { ...e };
    return Pt[a].forEach((n) => {
      delete i[n];
    }), i;
  }
}
async function Ut(e, a, i) {
  C == null || C.log("Start Engine in Main.... ");
  const n = `${e}_${Date.now()}`, s = new jn(a, n, C);
  let o = 0, t = 1;
  i ? i[0].params.hitsPerPage = 100 : C == null || C.error("Body is Undefined");
  do
    try {
      C == null || C.log(`Fetching page ${o}`), i[0].params.page = o;
      const { results: c } = await pn(e, i);
      if (c.length > 0) {
        const { hits: u, nbPages: r } = c[0];
        console.log("Nb Page", r), oa = qt(u, e), t = r, C == null || C.log(`Fetching page ${o} Done Out of ${t}`), s.writeData(oa);
      }
      o++;
      const d = (o / t * 100).toFixed(2);
      vi("progress", d), oa = [], o == t - 1 && vi("complete", "done");
    } catch (c) {
      console.error(c), C == null || C.error("Got Error in Main Loop");
    }
  while (o < t + 1);
...
class Nt {
  static async showError(a, i = "") {
    return await ya.showMessageBox({
      type: "error",
      title: "Error",
      message: a,
...
const ln = Z.dirname(hn(import.meta.url));
process.env.APP_ROOT = Z.join(ln, "..");
const ba = process.env.VITE_DEV_SERVER_URL, sr = Z.join(process.env.APP_ROOT, "dist-electron"), un = Z.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = ba ? Z.join(process.env.APP_ROOT, "public") : un;
let I;
function dn() {
  I = new gi({
    width: 1e3,
    height: 800,
    resizable: !1,
    icon: Z.join(process.env.VITE_PUBLIC, "logo.png"),
    webPreferences: {
      preload: Z.join(ln, "./preload.mjs"),
      nodeIntegration: !0,
      contextIsolation: !1
    }
  }), I.webContents.on("did-finish-load", () => {
    I == null || I.webContents.send("main-process-message", (/* @__PURE__ */ new Date()).toLocaleString());
  }), ba ? I.loadURL(ba) : I.loadFile(Z.join(un, "index.html")), I.setMenu(null);
}
Fe.on("window-all-closed", () => {
  process.platform !== "darwin" && (Fe.quit(), I = null);
});
Fe.on("activate", () => {
  gi.getAllWindows().length === 0 && dn();
});
Fe.whenReady().then(dn);
const ge = (e, a) => {
  I && !I.isDestroyed() && I.webContents.send("event", { Type: e, message: a });
};
Pe.handle("openPathDialog", async () => await ya.showOpenDialog({
  properties: ["openDirectory"]
}));
const ga = zt(ge), bi = (e) => e.toFixed(2), Dt = (e) => Math.round(e * 100);
function It(e, a) {
  const i = [
    {
      timestamp: "",
      level: "table",
      message: e === "articles" ? "Product | Brand | Category | Packaging | Tendency" : "Product | Store | Price | Discount | Price/Unit"
    },
    {
      timestamp: "",
      level: "table",
    ...
    if (e === "articles") {
      ...
      return {
        timestamp: "*",
        level: "table",
        message: `${o.name} | ${o.brand} | ${o.category} | ${o.packaging} | ${o.tendency_3m}%`
      };
    } else {
      const o = s, t = parseFloat(o.product_price) / 100, c = Dt(o.product_discount), d = parseFloat(o.product_unit_price) / 100;
      return {
        timestamp: "*",
        level: "table",
        message: `${o.article_name} | ${o.retailer} | ${bi(
          t
        )} lei | ${c}% | ${bi(d)} lei/${o.price_unit}`
      };
    }
  });
  return [...i, ...n];
...
Pe.handle(
  ...
  async (e, a, i) => {
    ...
    return o && ge("count", o), t && ge("data", It(a, t)), s;
  }
);
Pe.handle("show-file-path-error", async () => (await ya.showMessageBox({
  type: "error",
...
Pe.on(
  "start",
  async (e, a, i, n) => {
    ga.log("Staring . . . . "), console.log(xe);
    try {
      xe || (xe = !0, await Ut(a, n, i), xe = !1, ge("complete", !0));
    } catch (s) {
      console.error(s), ga.error(`Data fetching failed : ${s}`), Nt.showDataFetchError(), ge("complete", !0), xe = !1;
    }
  }
);
export {
