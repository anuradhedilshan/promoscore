...
class CSVWriter {
  ...
  constructor(filePath, name, logger2, type) {
    ...
    __publicField(this, "bufferSize", 16384);
    var _a;
   ...
   */
  objectToCSVLine(obj) {
    return this.headers.map((header) => this.escapeCSVValue(obj[header])).join(",") + "\n";
  }
  /**
  ...
  writeHeaders(data) {
    ...
    if (this.headers.length === 0) {
      ...
      const headerLine = this.headers.map((header) => this.escapeCSVValue(header)).join(",") + "\n";
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const processBatch = () => {
          ...
          const canContinue = batch.every(
            (item) => transformStream.write(item)
          );
        ...
        transformStream.on("error", (error) => {
          var _a3;
          (_a3 = this.logger) == null ? void 0 : _a3.error(`Transform stream error: ${error}`);
          reject(error);
        });
        processBatch();
      } catch (error) {
  ...
  async writeArrays(dataArrays) {
    ...
    try {
      ...
      (_a = this.logger) == null ? void 0 : _a.error(`Error writing arrays: ${error}`);
      throw error;
    }
  }
  /**
  ...
  async close() {
    ...
    return new Promise((resolve, reject) => {
      const cleanup = (error) => {
        ...
        this.headers = [];
        if (error) {
          (_a2 = this.logger) == null ? void 0 : _a2.error(`Error during close: ${error}`);
          reject(error);
        } else {
          resolve();
        }
      ...
      this.writeStream.end(() => {
        rename(this.tempFilePath, this.finalFilePath, (err) => {
          ...
          if (err) {
            ...
            (_a2 = this.logger) == null ? void 0 : _a2.log(`File renamed to ${this.finalFilePath}`);
            cleanup();
          }
        });
      });
...
const defaultRequestBodyArticle = [
  {
...
function Post(requestBody, contentType) {
  return axios.post(
    `https://promoscore.io/api/search/search-promoscore-${contentType == "articles" ? "articles" : "promotions"}`,
    requestBody,
    {
      headers: {
        ...HeadersS
...
function setLoggerCallback(cb) {
  f = cb;
  logger$1 = new Logger(cb);
  return logger$1;
}
function fireEvent$1(Type, message) {
  if (f) f(Type, message);
}
async function search_promoscore(type, body) {
  logger$1 == null ? void 0 : logger$1.log("searching _promoscore .....");
  if (!body) {
    body = type == "articles" ? defaultRequestBodyArticle : defaultRequestBodyPromotion;
  }
...
async function start(type, filePath, body) {
  ...
  const Writer = new CSVWriter(filePath, name, logger$1, type);
  let currentPage = 0;
  let totalPages = 1;
  if (body) {
    body[0].params.hitsPerPage = 100;
  } else {
    logger$1 == null ? void 0 : logger$1.error("Body is Undefined");
  }
  do {
    try {
...
class ErrorDisplay {
  static async showError(message, details = "") {
    const result = await dialog.showMessageBox({
      type: "error",
      title: "Error",
      message,
      detail: details || "Please try again. If the problem persists, contact support.",
      buttons: ["OK"],
      defaultId: 0,
      noLink: true
    });
...
const calculateDiscount = (discount) => Math.round(discount * 100);
function processProductData(type, data) {
  const headerLogs = [
    {
      timestamp: "",
      level: "table",
      message: type === "articles" ? "Product | Brand | Category | Packaging | Tendency" : "Product | Store | Price | Discount | Price/Unit"
    },
    {
      timestamp: "",
      level: "table",
    ...
    if (type === "articles") {
      ...
      return {
        ...
        message: `${promo.article_name} | ${promo.retailer} | ${formatPrice(
          price
        )} lei | ${discount}% | ${formatPrice(pricePerUnit)} lei/${promo.price_unit}`
      };
    }
  });
...
ipcMain.handle(
  "search_promoscore",
  async (_event, type, searchParams) => {
    ...
    if (nbHits) fireEvent("count", nbHits);
...
ipcMain.on(
  "start",
  async (_event, type, SearchRequest2, location) => {
    logger.log("Staring . . . . ");
    console.log(IS);
    try {
      if (!IS) {
        IS = true;
        await start(type, location, SearchRequest2);
        IS = false;
